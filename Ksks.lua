...
function Library.Main(Name)
    for _, 
   
v in pairs(game.CoreGui:GetChildren()) do
    ...
    local function CreateTab(TabName)
        ...
        MainElementList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            MainContainer.CanvasSize = UDim2.new(0, 0, 0, MainElementList.AbsoluteContentSize.Y + 4)
        end)
        ...
        BasicElementList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            BasicContainer.CanvasSize = UDim2.new(0, 0, 0, BasicElementList.AbsoluteContentSize.Y + 4)
        end)

        local isScrollingMain = false
        local isScrollingBasic = false
        local lastTouchPosMain = nil
        local lastTouchPosBasic = nil
        local scrollSpeed = 2
        local velocityMain = 0
        local velocityBasic = 0
        local lastDeltaMain = 0
        local lastDeltaBasic = 0
        local decay = 0.95
        local minVelocity = 5
        ...
        MainContainer.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch and not isDraggingUI then
                ...
                lastTouchPosMain = input.Position.Y
                velocityMain = 0
            end
        end)

        MainContainer.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch and isScrollingMain then
                local currentTouchPos = input.Position.Y
                lastDeltaMain = (lastTouchPosMain - currentTouchPos) * scrollSpeed
                local newCanvasPos = MainContainer.CanvasPosition.Y + lastDeltaMain
                newCanvasPos = math.clamp(newCanvasPos, 0, math.max(0, MainContainer.CanvasSize.Y.Offset - MainContainer.AbsoluteSize.Y))
                MainContainer.CanvasPosition = Vector2.new(0, newCanvasPos)
                velocityMain = lastDeltaMain
                lastTouchPosMain = currentTouchPos
            end
        end)
        ...
        MainContainer.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                ...
                if math.abs(velocityMain) > minVelocity then
                    spawn(function()
                        while math.abs(velocityMain) > minVelocity and not isScrollingMain do
                            local newCanvasPos = MainContainer.CanvasPosition.Y + velocityMain
                            newCanvasPos = math.clamp(newCanvasPos, 0, math.max(0, MainContainer.CanvasSize.Y.Offset - MainContainer.AbsoluteSize.Y))
                            MainContainer.CanvasPosition = Vector2.new(0, newCanvasPos)
                            velocityMain = velocityMain * decay
                            wait()
                        end
                        local finalPos = math.clamp(MainContainer.CanvasPosition.Y, 0, math.max(0, MainContainer.CanvasSize.Y.Offset - MainContainer.AbsoluteSize.Y))
                        TweenService:Create(MainContainer, TweenInfo.new(0.2), {CanvasPosition = Vector2.new(0, finalPos)}):Play()
                    end)
                end
            end
        end)

        BasicContainer.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch and not isDraggingUI then
                isScrollingBasic = true
                lastTouchPosBasic = input.Position.Y
                velocityBasic = 0
            end
        end)

        BasicContainer.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch and isScrollingBasic then
                local currentTouchPos = input.Position.Y
                lastDeltaBasic = (lastTouchPosBasic - currentTouchPos) * scrollSpeed
                local newCanvasPos = BasicContainer.CanvasPosition.Y + lastDeltaBasic
                newCanvasPos = math.clamp(newCanvasPos, 0, math.max(0, BasicContainer.CanvasSize.Y.Offset - BasicContainer.AbsoluteSize.Y))
                BasicContainer.CanvasPosition = Vector2.new(0, newCanvasPos)
                velocityBasic = lastDeltaBasic
                lastTouchPosBasic = currentTouchPos
            end
        end)

        BasicContainer.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                isScrollingBasic = false
                lastTouchPosBasic = nil
                if math.abs(velocityBasic) > minVelocity then
                    spawn(function()
                        while math.abs(velocityBasic) > minVelocity and not isScrollingBasic do
                            local newCanvasPos = BasicContainer.CanvasPosition.Y + velocityBasic
                            newCanvasPos = math.clamp(newCanvasPos, 0, math.max(0, BasicContainer.CanvasSize.Y.Offset - BasicContainer.AbsoluteSize.Y))
                            BasicContainer.CanvasPosition = Vector2.new(0, newCanvasPos)
                            velocityBasic = velocityBasic * decay
                            wait()
                        end
                        local finalPos = math.clamp(BasicContainer.CanvasPosition.Y, 0, math.max(0, BasicContainer.CanvasSize.Y.Offset - BasicContainer.AbsoluteSize.Y))
                        TweenService:Create(BasicContainer, TweenInfo.new(0.2), {CanvasPosition = Vector2.new(0, finalPos)}):Play()
                    end)
                end
            end
        end)
